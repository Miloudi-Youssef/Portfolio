  ...
  const handleSendMessage = async (
    ...
    try {
      ...
      if (error instanceof DOMException && error.name === 'AbortError') {
        ...
        updateLastMessage(() => ({
          role: 'assistant',
          content: 'An error occurred while processing your request. Please verify your connection and try again.',
          isStreaming: false,
          context_documents: effectiveContextDocs,
          context_document_labels: effectiveContextDocLabels,
        }))
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            <div className="w-20 h-20 bg-primary-100 dark:bg-primary-900/40 rounded-2xl flex items-center justify-center mb-6 shadow-sm">
              <svg
            ...
            <h2 className="text-xl font-semibold text-secondary-800 dark:text-secondary-200 mb-3">
              Analyse Your ESG Reports
            </h2>
            <p className="text-secondary-500 dark:text-secondary-400 max-w-lg text-sm leading-relaxed mb-6">
              Upload CSRD reports, sustainability disclosures, GRI filings, or TCFD assessments.
              Ask questions to extract structured ESG insights, compare metrics, and identify compliance gaps.
            </p>
            <div className="flex flex-wrap gap-2 justify-center max-w-lg">
              {[
                'What are our Scope 1 emissions?',
                'Summarise CSRD double materiality',
                'Extract GHG reduction targets',
                'Compare ESG performance year-over-year',
              ].map((suggestion) => (
                <span
                  key={suggestion}
                  className="px-3 py-1.5 text-xs bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-full border border-primary-200 dark:border-primary-800"
                >
                  {suggestion}
                </span>
              ))}
